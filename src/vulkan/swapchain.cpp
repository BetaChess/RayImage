...
aito::Swapchain::Swapchain(Device &deviceRef, VkExtent2D windowExtent)
		: device_(deviceRef), windowExtent_(windowExtent)
{
	init();
}

aito::Swapchain::Swapchain(aito::Device &deviceRef, VkExtent2D windowExtent, std::shared_ptr<Swapchain> previous)
		: device_(deviceRef), windowExtent_(windowExtent), oldSwapChain_(std::move(previous))
{
	init();
...
void aito::Swapchain::createSwapChain()
{
...
VkSurfaceFormatKHR aito::Swapchain::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR> &availableFormats)
{
...
VkPresentModeKHR aito::Swapchain::chooseSwapPresentMode(const std::vector<VkPresentModeKHR> &availablePresentModes)
{
...
VkExtent2D aito::Swapchain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities)
{
...
void aito::Swapchain::createImageViews()
{
	...
	for (size_t i = 0; i < swapChainImages_.size(); i++)
	{
		...
		if (vkCreateImageView(device_.device(), &viewInfo, nullptr, &swapChainImageViews_[i]) !=
			VK_SUCCESS)
		{
			AITO_ERROR("Failed to create texture image view");
			throw std::runtime_error("failed to create texture image view!");
		}
	}
}

void aito::Swapchain::createRenderPass()
{
	...
	{
		AITO_ERROR("Failed to create render pass");
		throw std::runtime_error("Failed to create render pass!");
	}
}

VkFormat aito::Swapchain::findDepthFormat()
{
...
void aito::Swapchain::createDepthResources()
{
...
void aito::Swapchain::createFramebuffers()
{
...
void aito::Swapchain::createSyncObjects()
{
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
	{
		...
		{
			AITO_ERROR("Failed to create synchronization objects for a frame");
			throw std::runtime_error("failed to create synchronization objects for a frame!");
		}
	}
}

VkResult aito::Swapchain::acquireNextImage(uint32_t *imageIndex)
{
	// The next image may already be in flight, so we wait for the fence to signal that the image is ready.
	vkWaitForFences(
			device_.device(),
...
VkResult aito::Swapchain::submitCommandBuffers(const VkCommandBuffer *buffers, uint32_t *imageIndex)
{
	...
	if (vkQueueSubmit(device_.graphicsQueue(), 1, &submitInfo, inFlightFences_[currentFrame_]) !=
		VK_SUCCESS)
	{
		AITO_ERROR("Failed to submit draw command buffer");
